...
export function ExchangeSelectorTop() {
      ...
      try {
        ...
        if (response.ok) {
          ...
          const allConnections: Connection[] = (data.connections || []).map((c: any) => ({
            ...
            isDashboardEnabled: c.is_enabled_dashboard === "1" || c.is_enabled_dashboard === true,
            isLiveTradeEnabled: c.is_live_trade === "1" || c.is_live_trade === true,
  ...
  const renderStatusBadges = () => {
    if (!currentConnection.isReal) {
      return (
        <Badge className="bg-muted text-muted-foreground text-xs">Mock Data</Badge>
      )
    }
    ...
    return (
      <>
        {currentConnection.isDashboardEnabled && (
          <Badge className="bg-green-500/20 text-green-600 border border-green-500/30 text-xs">
            Active
          </Badge>
        )}
        {currentConnection.isLiveTradeEnabled && (
          <Badge className="bg-blue-500/20 text-blue-600 border border-blue-500/30 text-xs">
            Live
          </Badge>
        )}
        {!currentConnection.isDashboardEnabled && currentConnection.status === "connected" && (
          <Badge className="bg-amber-500/20 text-amber-600 border border-amber-500/30 text-xs">
            Enabled
          </Badge>
        )}
      </>
